...
Test stack: Vitest + jsdom + `fake-indexeddb` (dev dependency). 150 tests as of #31.
...
flowchart LR
    ...
    subgraph Next ["⏭ Next"]
        RELOC["#34 Regenerate today's word on language switch, pre-memory (Tier 5)"]
    end
    Done --> Next
```
